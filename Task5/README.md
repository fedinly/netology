...
## Задача 5.
- [Скриншот выполнения команды - одноразовый бэкап БД](Ex5/screen-09.JPG). При этом в `comnpose.yaml` файле прописана инструкция в секции `db`: `command: --mysql-native-password=ON`. Пользователь БД должен быть создан с параметром `IDENTIFIED WITH mysql_native_password`. В скрипте бэкапа реализовать не удалось.
## Задача 6.  
