...
- По задаче создания бэкапов по расписанию было принято решение согласно документации к образу создать файл compose  и импортировать в текущую конфигурацию сервиса.  
...
С прописанной строкой `command: "--default-authentication-plugin=mysql_native_password"` контейнер не запускался, ошибка unknown variable [Скриншот](Ex5/screen-13.JPG).  
Была прописана строка в compose-файле `--mysql-native-password=ON` (из документации к mysql:8), при этом пользователь по умолчанию создался с методом аутентификации caching_sha2_password [Скриншот](Ex5/screen-14.JPG).  
В итоге также был создан пользователь с требуемым методом аутентификации, при инициализации БД, [Ссылка на мануал](https://github.com/mrmoghadasi/mysql-MySQL-Docker-Setup-with-Multiple-Users/tree/main), бэкапы создались 
## Задача 6. 
