...
При этом в созданном контейнере cron подключение утилитой mysqldump к БД не выполнялось, ошибка [Скриншот экрана](Ex5/screen-12.JPG)
С прописанной строкой `command: "--default-authentication-plugin=mysql_native_password"` контейнер не запускался, ошибка unknown variable [Скриншот экрана](Ex5/screen-13.JPG).
Была прописана строка в compose-файле `--mysql-native-password=ON` (из документации к mysql:8), при этом пользователь по умолчанию создался с методом аутентификации caching_sha2_password [Скриншот экрана](Ex5/screen-14.JPG).
## Задача 6. 
